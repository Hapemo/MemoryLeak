...
std::set<Entity> SerializationManager::LoadEntities(std::string const& _filePath) {
	std::ifstream ifs(_filePath);
	//std::ifstream ifs(filename);
	if (!ifs.good()) {
		LOG_ERROR("Can't load entities from " + _filePath);
		return std::set<Entity>();
	} else LOG_INFO("Loading entities from " + _filePath);
...
SceneData SerializationManager::LoadSceneData(std::string const& _filePath) {
	...
	if (!doc.HasMember(sceneDataJson)) std::cout << "No sceneData found in scene file!\n";
...
GameStateData SerializationManager::LoadGameStateData(std::string const& _filePath) {
	...
	if (!doc.HasMember(gamestateDataJson)) std::cout << "No gamestateData found in scene file!\n";

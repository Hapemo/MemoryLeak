...
namespace BonVoyage {
  public class LevelTutorialDialogueScript : BaseScript {
    ...
    public void Update(int _ENTITY) {
      // Dialog control
      PlayerScript.PlayerInDialogue = (runCatDialog || runGirlDialog || runPassengerDialog);
      if (!PlayerScript.PlayerInDialogue) return;

      if (runCatDialog) {
        runCatDialog = RunDialog("P1", "G1", "PP1", "PP2", "TutorialDialogue", "Dialogue SceneIntro 1");
        if (!runCatDialog) EndCatDialog();
      }
      ...
      if (runPassengerDialog) {
        VI.Animation.SheetIndex.Set(playerID, 1);
        runPassengerDialog = RunDialog("P1", "G1", "PP1", "PP2", "TutorialDialogue", "Dialogue Passenger 1");

...
bool Input::checkKey(STATE state, KEY key) {
  ...
  case 1: // Curr pressed
   ...
   default:
    ...
    assert(1 && "Invalid current key pressed");
...
void Input::updatePrevKeyStates() {
  for (size_t i = 0; i < sizeof(prev_key_states) - total_mouse_key; ++i)
    prev_key_states[i] = (bool)glfwGetKey(window, starting_index + i);
  for (size_t i = sizeof(prev_key_states) - total_mouse_key + 1, j = 0; i < sizeof(prev_key_states); ++i, ++j) {
